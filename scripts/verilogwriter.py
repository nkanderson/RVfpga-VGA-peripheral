...
class Wire(Signal):
    def write(self, width):
        return "wire{range} {name};\n".format(
            range=self.range().rjust(width), name=self.name
        )
...
class ModulePort(Signal):
    ...
    def write(self, range_width=0):
        return "{dir} {range} {name}".format(
            dir=self.dir.ljust(6), range=self.range().rjust(range_width), name=self.name
        )
...
class Instance:
    ...
    def write(self):
        ...
        if self.parameters:
            ...
            s += ",\n    ".join(
                [
                    "." + p.name.ljust(max_len) + " (" + str(p.value) + ")"
