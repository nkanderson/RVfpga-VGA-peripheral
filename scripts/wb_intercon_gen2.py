...
WB_SLAVE_PORTS = [Signal("dat", 32), Signal("ack"), Signal("err"), Signal("rty")]
        ...
        for key, value in d.items():
            if key == "slaves":
                ...
                raise UnknownPropertyError(
                    "Unknown property '%s' in master section '%s'" % (key, self.name)
                )
    ...
    def load_dict(self, d):
        for key, value in d.items():
            if key == "datawidth":
                ...
                raise UnknownPropertyError(
                    "Unknown property '%s' in slave section '%s'" % (key, self.name)
                )
...
class WbIntercon:
    def __init__(self, name, config_file):
        ...
        if "endian" in config:
            ...
            if self.endian not in valid_endians:
                raise UnknownPropertyError(
                    "Unknown data resizer endian '{}' specified. Valid endians: {}".format(
                        config["endian"], valid_endians
                    )
                )
        else:
            self.endian = "big"
    ...
    def _gen_mux(self, master):
        ...
        match_addr = (
            "{"
            + ", ".join(["32'h{addr:08x}".format(addr=s.offset) for s in master.slaves])
            + "}"
        )
        ...
        match_mask = (
            "{"
            + ", ".join(["32'h{mask:08x}".format(mask=s.mask) for s in master.slaves])
            + "}"
        )
    ...
    def _gen_arbiter(self, slave):
        ...
        # Else, connect to the resizer
            ...
            for p in WB_MASTER_PORTS:
                ports += [
                    Port("wbs_" + p.name + "_o", "wb_m2s_resize_" + s + "_" + p.name)
                ]
            for p in WB_SLAVE_PORTS:
                ports += [
                    Port("wbs_" + p.name + "_i", "wb_s2m_resize_" + s + "_" + p.name)
                ]
    ...
    def _gen_resize(self, slave):
        ...
        self.verilog_writer.add(
            Instance("wb_data_resize", "wb_data_resize_" + s, parameters, ports)
        )
    ...
    def _gen_bus_converter(self, bus, name, is_master, datawidth, datawidth_map, ports):
        converter_ports = [Port("wb_clk_i", "wb_clk_i"), Port("wb_rst_i", "wb_rst_i")]
        template_ports = []
        ...
        for p, direction in wb_ports:
            ...
            converter_ports.append(
                Port(
                    "%s_%s_%s"
                    % ("wbm" if ms_type == "m" else "wb", p.name, pin_direction[0]),
                    wirename,
                )
            )
        ...
        for p, direction in ports:
            ...
            portname = "{d}_{bus}_{name}_{port}_{direction}".format(
                d=f_ms_type, bus=bus, direction=pin_direction[0], name=name, port=p.name
            )
            ...
            converter_ports.append(
                Port(
                    "{d}_{bus}_{port}_{direction}".format(
                        d=f_ms_type, bus=bus, direction=pin_direction[0], port=p.name
                    ),
                    portname,
                )
            )
            dw = int(datawidth_map[p.name] * datawidth) or p.width
            self.verilog_writer.add(ModulePort(portname, pin_direction, p.width))
            self.template_writer.add(Wire(wirename, dw))
    ...
    def write(self):
            ...
            for slave in value.slaves:
                if len(slave.masters) > 1:
                    for p in WB_MASTER_PORTS:
                        self.verilog_writer.add(
                            Wire(
                                "wb_m2s_{0}_{1}_{2}".format(key, slave.name, p.name),
                                p.width,
                            )
                        )
                    for p in WB_SLAVE_PORTS:
                        self.verilog_writer.add(
                            Wire(
                                "wb_s2m_{0}_{1}_{2}".format(key, slave.name, p.name),
                                p.width,
                            )
                        )
        ...
        template_ports = [Port("wb_clk_i", "wb_clk"), Port("wb_rst_i", "wb_rst")]
        template_parameters = []
        ...
        with open(core_file, "w") as f:
            f.write("CAPI=2:\n")
            files = [
                {file: {"file_type": "verilogSource"}},
                {file + "h": {"is_include_file": True, "file_type": "verilogSource"}},
            ]
            coredata = {
                "name": vlnv,
                "targets": {"default": {}},
